...
describe('Repo model', () => {
  ...
  it('accepts params', () => {
    const repo = new Repo({ id: 1 });
    expect(repo.repo.id).to.equal(1);
  });
  it('fetchIssue method returns an issue', (done) => {
    const repo = new Repo({
      id: 95143360,
      name: 'skiski-app',
      full_name: 'ski-ski/skiski-app',
    ...
    repo.fetchIssue(() => {
      expect(Array.isArray(repoIssue)).to.equal(true);
      expect(repoIssue).to.have.length.above(1);
      expect(repoIssue[0].id).to.equal(247520833);
      console.log(repoIssue[0]);
    });
    done();
  });
  it('fetchPullRequest method returns a pull request', (done) => {
    const repo = new Repo({
      id: 95143360,
      name: 'skiski-app',
      full_name: 'ski-ski/skiski-app',
    ...
    repo.fetchPullRequest(() => {
      expect(Array.isArray(pr)).to.equal(true);
      expect(pr).to.have.length.above(1);
      expect(pr[0].id).to.equal();
    });
    done();
  });
});
