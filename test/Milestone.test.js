...
describe('Milestone model', () => {
  ...
  it('accepts id, title, open issues, closed issues, and due on as passed in arguments', () => {
    expect(milestone.id).to.equal(2677830);
    expect(milestone.title).to.equal('Test');
    expect(milestone.openIssues).to.equal(1);
    expect(milestone.closedIssues).to.equal(1);
    expect(milestone.dueOn).to.equal('2017-08-20T07:00:00Z');
  });
  after((done) => {
    done();
  });
